# Instruction Granularity as Planning Width: Formalizing and Benchmarking for Vision-Language-Action Models

This is the codebase for the paper *Instruction Granularity as Planning Width: Formalizing and Benchmarking for Vision-Language-Action Models*.

## ⚠️ Important Notice

**This codebase is not fully runnable** as we have masked out name-sensitive information for the anonymous review process, which may have broken some functionalities. Additionally, **training data is not provided** due to limited storage space constraints by the IJCAI submission system. 

**Full code and complete data will be made available upon paper acceptance.**

## Paper Overview

The paper formalizes instruction granularity for Vision-Language-Action (VLA) agents as planning demand, operationalized via rule width adapted from width-based planning. It introduces **Mini-BEHAVIOR-Gran**, a controlled extension of Mini-BEHAVIOR with per-task rule-set banks spanning up to six granularity levels and a dynamic oracle instructor that outputs valid step-wise natural language instructions. 

Using three action-decoding archetypes (autoregressive, discrete diffusion, parallel), the paper reports:
- A **U-shaped relationship** between instruction granularity and success rate
- An **asymmetry in generalization** (coarse→fine transfers better than fine→coarse)
- Evidence that coarse-only training can induce **language ignoring**, while mixed-granularity training (especially Axial-F/C) balances performance and grounding

## Repository Structure

### 1. `mini-behavior-gran/` - Mini-BEHAVIOR-Gran Environment

This folder contains the code to run experiments on the Mini-BEHAVIOR-Gran dataset. We use **PDDL (Planning Domain Definition Language)** format to represent the state of the environment at the symbolic level.

#### Key Highlights:

- **PDDL Domain Model**: We have invested significant effort in designing the comprehensive PDDL domain model [`minibehavior_gran_domain_model.pddl`](minibehavior_gran_domain_model.pddl) that covers the full range of tasks in the Mini-BEHAVIOR-Gran dataset.

- **Problem Instances and Reference Plans**: The symbolic representations of problem instances and their corresponding reference plans are stored in `pddl_files_for_mini_behavior_gran.zip`.

#### Key Subdirectories:

- **`mini-behavior-gran/mini_behavior/utils/pddl_gen/`**  
  Contains code for generating problem instances and reference plans.

- **`mini-behavior-gran/mini_behavior/utils/policy_sketch/`**  
  Stores the instruction rule banks for different granularity levels.

- **`mini-behavior-gran/mini_behavior/utils/policy_sketch/general_policy_sketch.py`**  
  Implements the **dynamic oracle instructor** that generates valid step-wise natural language instructions.

### 2. `training_with_varying_instruction_granularities/` - VLA Training Pipeline

This folder contains the code to train Vision-Language-Action models with varying instruction granularities on the Mini-BEHAVIOR-Gran dataset.

#### Key Components:

- **`experiments/robot/mini_behavior/regenerate_mini_behavior_dataset_based_on_width.py`**  
  Converts symbolic representations of problem instances into actual **pixel-based observations and language training data** for VLA models.

- **`scripts/mini_behavior_gran/`**  
  Contains example scripts to train VLA models on Mini-BEHAVIOR-Gran dataset with varying instruction granularities and training settings.

- **`eval/get_eval_result_instruction_split.py`**  
  Evaluation script to assess trained VLA models on Mini-BEHAVIOR-Gran dataset with varying instruction granularities.

## Training VLA Models

### Training Command Example

To train a VLA model, use the following command:

```bash
bash scripts/mini_behavior/run_single.sh --server_name xxxorgan --data_root_dir default --dataset_name mini_behavior_single_instr_pure_M --gpus 0,1
```

**Parameters:**
- `--server_name`: Server configuration name
- `--data_root_dir`: Root directory for dataset (use `default` for default path)
- `--dataset_name`: Name of the dataset configuration
- `--gpus`: GPU indices to use (comma-separated)

### Evaluation Command Example

To evaluate a trained model (requires interaction with the Mini-BEHAVIOR environment):

```bash
python ./eval_scripts/get_eval_result_original.py
```

**Note:** If you have questions about installing the Mini-BEHAVIOR environment, please contact xxxname.

## Running OpenVLA OFT on Spartan Multi-Node

### Important Configuration Notes

1. **Weights & Biases Setup**: You **must** set the `wandb_entity` to your WANDB username. It cannot be left as another name.

2. **Model Path**: Use `--vla_path openvla/openvla-7b`, but you **must download the model first**. Otherwise, multi-node torchrun will fail as both nodes will try to download the model simultaneously.
...
export PYTHONPATH=/data/projects/xxxplace0478/xxxnameh/xxxname_Project/vla/SimpleVLA-RL/modules/openvla-oft
export WORKING_DIR=/data/projects/xxxplace0478/xxxnameh/xxxname_Project/vla/SimpleVLA-RL/modules/openvla-oft
export WANDB_DIR=/scratch/xxxplace0478/xxxnameh/wandb
export HF_HOME=/data/projects/xxxplace0478/xxxnameh/huggingface
export TRITON_CACHE_DIR=/data/projects/xxxplace0478/xxxnameh/triton_cache
...
- Training for **150K steps** takes approximately **80 hours** on 2 nodes with 4 GPUs each (8 GPUs total).
