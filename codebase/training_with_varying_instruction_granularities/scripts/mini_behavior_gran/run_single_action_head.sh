...
USE_DIFFUSION=false # ! do not change this to true
# if USE_DIFFUSION is true, USE_L1_REGRESSION should be false
if [ "$USE_DIFFUSION" = true ]; then
...
# if USE_INSTRUCTION_SENTENCE_ENCODER or USE_TEXT_WORLD_STATE_MODEL is true, reduce batch size by 1
if [ "$USE_INSTRUCTION_SENTENCE_ENCODER" = true ] || [ "$USE_TEXT_WORLD_STATE_MODEL" = true ]; then
...
DIFFUSION_PART=""
[ "$USE_DIFFUSION" = true ] && DIFFUSION_PART="-with_diffusion" || DIFFUSION_PART=""

RUN_ID_NOTE="finetune-${ENCODER_PART}-chunks${NUM_CHUNKS_FOR_TEXT}-${WSM_PART}${DIFFUSION_PART}"
