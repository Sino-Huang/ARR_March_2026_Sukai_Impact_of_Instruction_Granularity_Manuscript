...
from prismatic.vla.datasets.datasets import group_string_into_n_strings, split_by_double_newline, split_by_please_token
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    if cfg.use_text_world_state_model:
        ...
        vla = _apply_text_world_state_model_to_vla(vla, cfg)

    # Set number of images in model input
    # check if set_num_images_in_input in vision_backbone
...
def _apply_text_world_state_model_to_vla(vla: torch.nn.Module, cfg: Any) -> torch.nn.Module:
    """
    Apply Text-based World State Model to the VLA vision backbone.

    Args:
        vla: The VLA model
        cfg: Configuration object with model parameters
    Returns:
        torch.nn.Module: VLA model with Text-based World State Model applied
    """
    ...
    vla = get_peft_model(vla, lora_config) # ! actually it seems that this is a redundant step because the loaded vla is already having the merge_and_unload called
    
    # create and apply Text-based World State Model vision backbone
    new_vision_backbone = TextWorldStatePrismaticVisionBackbone(num_chunks=cfg.num_chunks_for_text)
    vla.model.vision_backbone = new_vision_backbone
    
    # Load vision backbone checkpoint
    # checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "vision_backbone_text_world_state")
    # state_dict = torch.load(checkpoint_path, weights_only=True)
    # vla.model.vision_backbone.load_state_dict(state_dict)
    # no load because we are freezing the weights
    ...
    vla.model.projector = projector
    # load the projector checkpoint
    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "projector_text_world_state")
    state_dict = torch.load(checkpoint_path, weights_only=True)
    ...
    vla.projector = vla.projector.to(torch.bfloat16)
    return vla
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead]:
    ...
    if model_is_on_hf_hub(cfg.pretrained_checkpoint):
        ...
        print(f"Find Action Head Checkpoint {checkpoint_path}")
        state_dict = load_component_state_dict(checkpoint_path)
        action_head.load_state_dict(state_dict)

    return action_head

def get_instruction_sentence_encoder(cfg: Any, llm_dim: int) -> InstructionSentenceEncoder:
    """
    Get instruction sentence encoder for processing textual instructions.

    Args:
        cfg: Configuration object with model parameters
        llm_dim: Dimension of the language model
    Returns:
        InstructionSentenceEncoder: The initialized instruction sentence encoder
    """
    # Initialize 

    instruction_sentence_encoder = InstructionSentenceEncoder(
        num_chunks=cfg.num_chunks_for_text,
        projector_output_dim=llm_dim,
    )


    instruction_sentence_encoder = instruction_sentence_encoder.to(torch.bfloat16).to(DEVICE)
    instruction_sentence_encoder.eval()

 
    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "light_sentence_encoder_projector")
    # print it 
    light_projector_state_dict = load_component_state_dict(checkpoint_path)
    instruction_sentence_encoder.light_projector.load_state_dict(light_projector_state_dict)

    return instruction_sentence_encoder
...
def get_vla_action(
    ...
    processor: Any,
    obs: Dict[str, Any],
    task_label: str, # this is the current instruction
    ...
    sentence_encoder_pad_token_id: Optional[int] = None,
    instruction_sentence_encoder: Optional[torch.nn.Module] = None,
    vision_backbone_sentence_encoder_tokenizer: Optional[Any] = None,
    language_instruction_sentence_encoder_tokenizer: Optional[Any] = None,
    text_world_state_label: Optional[str] = None,
    ...
    """
    # need to support text based world state model and instruction encoder here
    with torch.inference_mode():
        ...
        # Build VLA prompt
        # task_label can be another one if vision_backbone_sentence_encoder_tokenizer is not None
        if language_instruction_sentence_encoder_tokenizer is None:
            prompt = f"In: What action should the robot take to {task_label.lower()}?\nOut:"
            compressed_lang = None
        else:
            prompt = f"In: What action should the robot take to achieve the mission?\nOut:"
            compressed_lang = task_label.lower()
            compressed_lang = group_string_into_n_strings(
                raw_string = compressed_lang, 
                raw_split_func = split_by_please_token, 
                N = num_chunks_for_text, 
            )
            
        if compressed_lang is not None:
            compressed_input_dict = language_instruction_sentence_encoder_tokenizer(
                compressed_lang,
                padding=True,
                truncation=True,
                max_length=2048,
                return_tensors="pt",
            ) # (batch_size * num_chunks_for_text, seq_len)
            
        # handle text based world state model
        if vision_backbone_sentence_encoder_tokenizer is not None:
            text_world_state = text_world_state_label.lower() 
            state_collection = text_world_state.split('\n\n')
            text_world_state = '\n\n'.join(state_collection[:-1])
            position_info_str = state_collection[-1]
            prompt_split = prompt.split('\nOut:')
            prompt = f"{prompt_split[0]}\n{position_info_str}\nOut:"
            
            text_world_state = group_string_into_n_strings(
                raw_string=text_world_state,
                raw_split_func= split_by_double_newline,
                N=num_chunks_for_text,
            )
            text_world_state_dict = vision_backbone_sentence_encoder_tokenizer(
                text_world_state,
                padding=True,
                truncation=True,
                max_length=2048,
        ...
        # check the shape of inputs["pixel_values"]
        # print(f'Primary image pixel values shape in openvla_utils: {inputs["pixel_values"].shape}')
        # print(f'input_ids shape in openvla_utils: {inputs["input_ids"].shape}')
        # Primary image pixel values shape in openvla_utils: torch.Size([1, 6, 224, 224])                                                                                                                           
        # input_ids shape in openvla_utils: torch.Size([1, 57])  
        ...
        # generate text_world_state_input_ids and text_world_state_attention_mask if needed
        if use_text_world_state_model:
            # text_world_state_input_ids was settled in prismatic/util/data_utils.py in training time. 
            assert vision_backbone_sentence_encoder_tokenizer is not None
            text_world_state_input_ids = text_world_state_dict.input_ids.to(DEVICE)
            text_world_state_attention_mask = text_world_state_dict.attention_mask.to(DEVICE)
        else:
            text_world_state_input_ids = None
            text_world_state_attention_mask = None
        
        # generate compressed_instruction_input_embeds if needed
        if use_instruction_sentence_encoder:
            assert compressed_lang is not None
            # similar construction can be found in prismatic/vla/datasets/datasets.py
            compressed_input_ids = compressed_input_dict.input_ids # shape (batch_size * num_chunks_for_text, seq_len)
            compressed_attention_mask = compressed_input_dict.attention_mask
            
            input_dict = {
                "input_ids":compressed_input_ids.to(DEVICE),
                "attention_mask":compressed_attention_mask.to(DEVICE),
            }
