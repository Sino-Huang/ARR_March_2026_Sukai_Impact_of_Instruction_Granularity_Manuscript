...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def __init__(self, config: PrismaticConfig) -> None:
        ...
        # use_fused_vision_backbone is often False for openvla-oft 
        # image_size is [224] for openvla-oft
        # timm model is vit_so400m_patch14_siglip_224
        # timm_override_act_layers is null for openvla-oft
        self.vision_backbone = PrismaticVisionBackbone(
            config.use_fused_vision_backbone, config.image_sizes, config.timm_model_ids, config.timm_override_act_layers
        )

        # Create Multimodal Projector
        # * this is a simple 2 layer MLP projector from vision backbone dim to llm dim
        print("Config use_fused_vision_backbone:", config.use_fused_vision_backbone)
    ...
    def _process_vision_features(self, pixel_values, language_embeddings=None, use_film=False, use_text_world_state_model=False, text_world_state_input_ids=None, text_world_state_attention_mask=None, num_chunks_for_text=None):
        """Process vision features with optional FiLM conditioning"""
        if not use_text_world_state_model:
            if use_film:
            ...
            assert text_world_state_input_ids is not None, "text_world_state_input_ids must be provided when using text_world_state_model"
            assert num_chunks_for_text is not None, "num_chunks_for_text must be provided when using text_world_state_model"
            input_dict = {
                "input_ids": text_world_state_input_ids,
                "attention_mask": text_world_state_attention_mask,
            }
            patch_features = self.vision_backbone(input_dict)
            # Reshape patch_features to (bsz, num_chunks_for_text, D), currently is (bsz * num_chunks_for_text, D)
            last_dim_size = patch_features.size(-1)
            patch_features = patch_features.view(-1, num_chunks_for_text, last_dim_size)
    ...
    # ! add DISCRETE DIFFUSION related functions below
    ...
    def forward(
        ...
        num_chunks_for_text: Optional[int] = None,
    ) -> Union[Tuple, PrismaticCausalLMOutputWithPast]:
        # deal with use_text_world_state_model here to replace the vision backbone
        ...
        # print(f'use_text_world_state_model: {use_text_world_state_model}, input_ids.shape[0]: {input_ids.shape[0]}, num_chunks_for_text: {num_chunks_for_text}, text_world_state_input_ids.shape[0]: {text_world_state_input_ids.shape[0] if text_world_state_input_ids is not None else "N/A"}')
        ...
        elif pixel_values is None and not use_text_world_state_model:
        ...
        # === Handle Multimodal Forward ===
        elif (not use_text_world_state_model and ((input_ids.shape[0] == pixel_values.shape[0]) or (inputs_embeds.shape[0] == pixel_values.shape[0]))) or (use_text_world_state_model and (int(input_ids.shape[0] * num_chunks_for_text) == text_world_state_input_ids.shape[0])):
            assert past_key_values is None, "Unexpected key `past_key_values` provided during multimodal forward!"

            # Get input embeddings (from language model embeddings)
            input_embeddings = self.get_input_embeddings()(input_ids)  # (B, seq_len, D), the seq_len includes action tokens
            ...
            # Get visual features
            projected_patch_embeddings = self._process_vision_features(pixel_values, language_embeddings, use_film, use_text_world_state_model, text_world_state_input_ids, text_world_state_attention_mask, num_chunks_for_text)
            ...
            # add compressed input ids embeddings if asked
            if use_instruction_sentence_encoder:
                assert compressed_instruction_input_embeds is not None, "compressed_instruction_input_embeddings must be provided when using instruction sentence encoder"
                # view the shape of compressed_instruction_input_embeddings
                B = projected_patch_embeddings.shape[0]
                last_dim = compressed_instruction_input_embeds.shape[-1]
                temp_embeddings = compressed_instruction_input_embeds.view(B, -1, last_dim)
                projected_patch_embeddings = torch.cat(
                    (projected_patch_embeddings, temp_embeddings), dim=1
                )
            ...
            if noisy_actions is not None:
                ...
                input_embeddings = input_embeddings * ~all_actions_mask # Zero out action token embeddings
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _unnormalize_actions(self, normalized_actions, unnorm_key=None):
        ...
        if ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.NORMAL:
            mean = np.asarray(action_norm_stats["mean"])
            mask = np.asarray(action_norm_stats.get("mask", np.ones_like(mean, dtype=bool)))

            # Shortcut: nothing to unnormalize
            if not np.any(mask):
                return normalized_actions
            
            std = np.asarray(action_norm_stats.get("std", action_norm_stats.get("stdev")))
            

            # Inverse of (x - mean) / (std + eps)
            return np.where(mask, normalized_actions * (std + 1e-8) + mean, normalized_actions)
    ...
    def _run_diffusion_prediction(
        ...
        return curr_noisy_actions.float().cpu().detach().numpy(), actions_hidden_states, attentions
    ...
    def _regression_or_discrete_prediction(
        ...
        return normalized_actions, actions_hidden_states, attentions
    ...
    def _discrete_diffusion_prediction(
        ...
        if action_head is not None:
            ...
            def tokens_to_logits(suffix_seq: torch.LongTensor) -> torch.Tensor:
                ...
                return full_logits, actions_hidden_states, attentions          # suffix 部分 [B, L, V]
            ...
            final_iters, actions_hidden_states, first_attentions = parallel_decode.decode(
                init_ids=cur_seqs,
        ...
        return normalized_actions, actions_hidden_states, first_attentions

    # * this function is called by `get_vla_action` in experiments/robot/openvla_utils.py
    def predict_action(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        ...
        **kwargs: str,
        ...
        projected_patch_embeddings = self._process_vision_features(pixel_values, language_embeddings, use_film,
                                                                   use_text_world_state_model, text_world_state_input_ids,
                                                                   text_world_state_attention_mask, num_chunks_for_text)
        ...
        if use_instruction_sentence_encoder:
            assert num_chunks_for_text is not None, "num_chunks_for_text must be provided when using instruction sentence encoder"
            NUM_PATCHES += num_chunks_for_text
        ...
        if use_diffusion:
            ...
            if use_discrete_diffusion:
                normalized_actions, actions_hidden_states, attentions = self._discrete_diffusion_prediction(
                    input_embeddings,
                    all_actions_mask,
                    projected_patch_embeddings,
                    attention_mask,
        ...
        if output_attentions:
            ...
            return actions, actions_hidden_states, attention_dict
        else:
