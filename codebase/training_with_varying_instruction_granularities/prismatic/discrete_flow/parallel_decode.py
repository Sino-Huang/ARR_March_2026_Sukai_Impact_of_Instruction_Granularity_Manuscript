...
def decode(
    ...
    first_attentions = None
    # 迭代解码
    for step in range(start_iter, num_iter):
        # 1) 得到 logits & 概率分布
        logits, actions_hidden_states, attentions = tokens_to_logits(cur_seqs)             # [B, L, V]
        if first_attentions is None:
            first_attentions = attentions
    ...
    return final_seqs, actions_hidden_states, first_attentions
