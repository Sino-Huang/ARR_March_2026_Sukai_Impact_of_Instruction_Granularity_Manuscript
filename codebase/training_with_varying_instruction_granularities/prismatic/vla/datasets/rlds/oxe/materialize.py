...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] not in [ActionEncoding.EEF_POS, ActionEncoding.EEF_R6, ActionEncoding.JOINT_POS_BIMANUAL, ActionEncoding.OVERCOOKED_ACT, ActionEncoding.MINI_BEHAVIOR_ACT]:
        raise ValueError(f"Cannot load `{dataset_name}`; only EEF_POS & EEF_R6 & JOINT_POS_BIMANUAL & OVERCOOKED_ACT & MINI_BEHAVIOR_ACT actions supported!")
    ...
    if dataset_kwargs["action_encoding"] is ActionEncoding.OVERCOOKED_ACT:
        # all not normalize
        dataset_kwargs['absolute_action_mask'] = [True] * ACTION_DIM
        dataset_kwargs['action_normalization_mask'] = [False] * ACTION_DIM
    elif dataset_kwargs["action_encoding"] is ActionEncoding.MINI_BEHAVIOR_ACT:
        # all not normalize
        dataset_kwargs['absolute_action_mask'] = [True] * ACTION_DIM
        dataset_kwargs['action_normalization_mask'] = [False] * ACTION_DIM
    elif dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POS:
    ...
    if load_text_based_world_state:
        dataset_kwargs["text_based_world_state_key"] = "text_world_state"
