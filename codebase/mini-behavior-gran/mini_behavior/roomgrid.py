...
class RoomGrid(MiniBehaviorEnv):
        ...
        for room_id, room in enumerate(self.room_instances):
            ...
            for fur_id in range(num_furs):
                if fur_id < len(furniture_initial) and furniture_initial[fur_id]['type'] is not None:
                    furniture_type = furniture_initial[fur_id]['type'] # ! this is where table, chair, apple types comining in 
                else:
                ...
                if furniture_type in OBJECT_CLASS.keys():
                    fur_instance = OBJECT_CLASS[furniture_type](
                        name=fur_name) # Ashcan, Car are in its dedicated class, while others like Apple and other foods are in WorldObj class
                else:
    ...
    def _gen_grid(self, width, height):
        if not self.init_dict:
            ...
            self.place_agent()
            self.connect_all()
        else: # if init_dict is provided, we will use it to generate the grid
            self.grid = BehaviorGrid(width, height)
            self._gen_floorplan()
            self._gen_random_objs() # generate objects based on the floor plan (not "random" but based on the init_dict)
