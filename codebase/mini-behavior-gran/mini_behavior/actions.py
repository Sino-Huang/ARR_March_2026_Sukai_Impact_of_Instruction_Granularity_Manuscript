...
class Drop(BaseAction): # drop can drop on top of a furniture, or on the floor 
    def __init__(self, env):
...
class DropIn(BaseAction): # Dropin can drop inside an container furniture 
    def __init__(self, env):
        ...
        for i in range(3):
            ...
            if obj is None and furniture is not None and furniture.can_contain and i in furniture.can_contain:
                if 'openable' not in furniture.states or furniture.check_abs_state(self.env, 'openable'):
                    if last_obj is not None: # this means something must already stay below of the dimension you want to drop in
                        dims.append(i)
...
class Pickup(BaseAction):
    ...
    def do(self, obj):
        ...
        # figure out what container (if any) we're in
        fwd_pos   = self.env.front_pos
        container = self.env.grid.get_furniture(*fwd_pos, dim)
        middle_container = self.env.grid.get_furniture(*fwd_pos, 1)

        # remove the picked object from the grid
        self.env.grid.remove(*obj.cur_pos, obj)
        
        # check if the obj was inside a container
        new_objs = objs[:dim] # objects that are above the picked object
        candidate_objs = objs[dim + 1:]  # objects below the picked object
        obj_inside = obj.check_rel_state(self.env, container, 'inside')
        if container is not None:
            if obj_inside:
                # shift the remaining objects in the container
                for cand_i , candidate_obj in enumerate(candidate_objs):
                    if candidate_obj is None:
                        new_objs.append(None)
                    elif candidate_obj.check_rel_state(self.env, container, 'inside'):
                        # shift happens
                        new_objs.append(candidate_obj)
                    else:
                        # no shift and append None 
                        new_objs.append(None)
                        new_objs.extend(candidate_objs[cand_i:])
                        break
            else:
                # if the object was not inside the container, shift the objects not inside the container
                for cand_i , candidate_obj in enumerate(candidate_objs):
                    if candidate_obj is None:
                        new_objs.append(None)
                    elif candidate_obj.check_rel_state(self.env, container, 'inside'):
            ...
            # add special case, if container is a Table, then no shift happens
            if middle_container is not None and type(middle_container).__name__ == 'Table':
                # no shift happens
                new_objs.append(None)
                new_objs.extend(candidate_objs)
            else:
                # if the object was not inside a container, shift always happens
        ...
        if len(new_objs) < 3:
            new_objs.extend([None] * (3 - len(new_objs)))
        
        self.env.grid.set_all_objs(*obj.cur_pos, new_objs)

        # drop the "inside" flag on this object
        if obj_inside:
            obj.states['inside'].set_value(container, False)

        # update the object's position to “not on the grid”
