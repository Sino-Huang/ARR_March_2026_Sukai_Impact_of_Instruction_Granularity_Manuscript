...
class CollectMisplacedItemsEnv(RoomGrid):
    ...
    def _gen_objs(self):
        ...
        # table_0_pos = self._rand_subset(avail_pos_helper(tables[0], self), 1)[0]
        # self.put_obj(gym_shoe, *table_0_pos, 0)

        cabinet_pos = self._rand_subset(avail_pos_helper(cabinet, self), 1)[0]
        self.put_obj(gym_shoe, *cabinet_pos, 2)

        table_1_pos = self._rand_subset(avail_pos_helper(tables[1], self), 2)[0]
    ...
    def _end_conditions(self):
        ...
        for obj in gym_shoe + necklace + notebook + socks:
            ontop_condition = obj.check_rel_state(self, tables[1], 'onTop') or obj.check_rel_state(self, tables[0], 'onTop')
            if not ontop_condition:
                return False
...
register(
    id='MiniGrid-CollectMisplacedItems-14x14-N2-v0',
    entry_point='mini_behavior.envs:CollectMisplacedItemsEnv',
    kwargs={'room_size': 14}
)
