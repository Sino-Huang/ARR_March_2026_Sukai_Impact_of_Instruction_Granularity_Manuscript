...
class MakingTeaEnv(RoomGrid):
    ...
    def _gen_objs(self):
        ...
        pos_1, pos_2, pos_3 = self._rand_subset(avail_pos_helper(cabinet, self), 3)
        ...
        pos = self._rand_subset(avail_pos_helper(electric_refrigerator, self), 1)[0]
        self.put_obj(lemon, *pos, 1)
        lemon.states['inside'].set_value(electric_refrigerator, 'inside')
        
        # set lemon not sliceable at the beginning
        lemon.states['sliceable'].set_value(False)
...
register(
    id='MiniGrid-MakingTea-12x12-N2-v0',
    entry_point='mini_behavior.envs:MakingTeaEnv',
    kwargs={'room_size': 12}
)
