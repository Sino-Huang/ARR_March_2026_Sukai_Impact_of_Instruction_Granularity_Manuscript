...
class OrganizingFileCabinetEnv(RoomGrid):
    ...
    def _gen_objs(self):
        ...
        pos_0, pos_1, pos_2 = self._rand_subset(avail_pos_helper(table, self), 3)
        pos_3, pos_4 = self._rand_subset(avail_pos_helper(cabinet, self), 2)
        ...
        self.put_obj(documents[1], *pos_3, 0) # cabinet need to be bottom due to gravity
        # set value 
        documents[1].states['inside'].set_value(cabinet, True)
        self.put_obj(documents[2], *pos_1, 2)
        self.put_obj(documents[3], *pos_4, 0)
        documents[3].states['inside'].set_value(cabinet, True)
...
register(
    id='MiniGrid-OrganizingFileCabinet-10x10-N2-v0',
    entry_point='mini_behavior.envs:OrganizingFileCabinetEnv',
    kwargs={'room_size': 10}
)
