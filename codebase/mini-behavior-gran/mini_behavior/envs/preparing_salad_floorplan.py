    ...
    def _end_conditions(self):
        ...
        def forpair_slice(vegs):
            for veg in vegs:
                forpair = False
                for plate in plates:
                    if plate.check_abs_state(self, 'sliceable') and veg.check_rel_state(self, plate, 'onTop'):
                        forpair = True
                        break
                if not forpair:
                    return False
            ...
            for plate in plates:
                forpair = False
                for veg in vegs:
                    if veg.check_abs_state(self, 'sliceable') and veg.check_rel_state(self, plate, 'onTop'):
                        forpair = True
                        break
                if not forpair:
                    return False
