...
class MiniBehaviorEnv(MiniGridEnv):
    ...
    def reset(self):
        ...
        self._gen_grid(self.width, self.height) # ! NOTE: this is where the objects are stated

        self.update_states() # ! NOTE: after this, the self.grid.state_values is generated, it is a dictionary where the key is the object instance and the value is the list of ability values
        # generate furniture view
        self.furniture_view = self.grid.render_furniture(tile_size=TILE_PIXELS, obj_instances=self.obj_instances) # this is pixel array of the furniture view
    ...
    def update_states(self):
        for obj in self.obj_instances.values():
            for name, state in obj.states.items(): # these states are insameroomasrobot etc. 
    ...
    def render_states(self, tile_size=TILE_PIXELS, want_print=True):
        pos = self.front_pos
        imgs = []
