    ...
    def _update(self, env):
        ...
        for tool_type in self.tools:
            for water_source in env.objs.get(tool_type, []):
                if water_source.check_abs_state(env, 'toggleable') or water_source.type == 'teapot': # ! edit, teapot is not toggleable
...
class Inside(RelativeObjectState):
    ...
    def _set_value(self, other, new_value):
        if new_value:
            ...
            other.contains = self.obj # ! NOTE: this is buggy, a container can contain multiple objects, but only the last one is recorded
        else:
...
class OnTop(RelativeObjectState):
    ...
    def _get_value(self, other, env=None):
        if other is None or self.obj == other:
            # print("OnTop: other is None or self.obj == other")
            return False

        if self.obj.check_abs_state(self, 'inhandofrobot'):
            # print("OnTop: self.obj is in hand of robot")
            return False
        ...
        if other not in cell:
            # print(f"OnTop: {other.name} not in cell of {obj.name}")
            return False

        other_idx = cell.index(other)

        if obj_idx >= 0 and other_idx >= 0:
            # print(f"OnTop: obj_idx={obj_idx}, other_idx={other_idx}")
            # print(f"OnTop: {obj.name} is on top of {other.name}: {obj_idx < other_idx}")
