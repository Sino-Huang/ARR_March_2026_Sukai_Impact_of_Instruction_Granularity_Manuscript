...
- $H$: “holding an unstored cookable item (food)”
- $p$: distance to the nearest unstored cookable item
- $s$: distance to a cabinet
- $is\_{opened}$: whether the cabinet is opened
- $N_f$: number of unstored cookable items


### Sketch with W = 0 (approximate)

$\{\neg H, N_f > 0, p > 0\} \mapsto \{p \downarrow, s ?\}$  ; move toward the nearest unstored cookable item
$\{\neg H, N_f > 0, p = 0\} \mapsto \{H, s ?\}$  ; pick up the cookable item when reachable
$\{H, N_f > 0, s > 0\} \mapsto \{s \downarrow, p ?\}$  ; move toward the cabinet
$\{\neg is\_opened, s = 0\} \mapsto \{is\_opened, p ?\}$  ; open the cabinet when reachable
$\{H, N_f > 0, is\_opened, s = 0\} \mapsto \{\neg H, N_f \downarrow, p=0\}$  ; place the cookable item into the cabinet when at the spot
"""
...
def create_width_0_sketch(env, map_id, domain_name, window=None):
    ...
    # 1. move toward the nearest unstored cookable item
    # * precons
    neg_H = Holding(
        wanted_value=False,
        type_tag='chip oatmeal sugar vegetable_oil',
    )
    ...
    rule_move_to_cookable = Rule(
        preconditions=[neg_H, n_f_larger_0, p_larger_0],
        effects=[p_down],
        goal_clause_pattern="""
(inreachofrobot agent-01 {chip oatmeal sugar vegetable_oil})
"""
    )
    sketch_dict["storing food"].append(rule_move_to_cookable)
    
    # 2. pick up the cookable item when reachable
    # * precons
    neg_H = Holding(
        wanted_value=False,
        type_tag='chip oatmeal sugar vegetable_oil',
    ...
    rule_pick_up_cookable = Rule(
        preconditions=[neg_H, n_f_larger_0, p_equal_0],
        effects=[H],
        goal_clause_pattern="""
(inhandofrobot agent-01 {chip oatmeal sugar vegetable_oil})
"""
    )
    sketch_dict["storing food"].append(rule_pick_up_cookable)
    
    # 3. move toward the cabinet
    # * precons
    H = Holding(
        wanted_value=True,
        type_tag='chip oatmeal sugar vegetable_oil',
    ...
    rule_move_to_cabinet = Rule(
        preconditions=[H, n_f_larger_0, s_larger_0],
        effects=[s_down],
        goal_clause_pattern="""
(inreachofrobot agent-01 {cabinet})
"""
    )
    sketch_dict["storing food"].append(rule_move_to_cabinet)
    # 4. open the cabinet when reachable
    # * precons
    ...
    rule_open_cabinet = Rule(
        preconditions=[neg_is_opened, s_equal_0],
        effects=[is_opened],
        goal_clause_pattern="""
(is-opened {cabinet})
"""
    )
    sketch_dict["storing food"].append(rule_open_cabinet)
    
    # 5. place the cookable item into the cabinet when at the spot
    # * precons
    H = Holding(
        wanted_value=True,
        type_tag='chip oatmeal sugar vegetable_oil',
    ...
    rule_place_cookable_in_cabinet = Rule(
        preconditions=[H, n_f_larger_0, is_opened, s_equal_0],
        effects=[neg_H, n_f_decrease],
        goal_clause_pattern="""
(exists (?loc - location ?dim - dimension)
    (inside {chip oatmeal sugar vegetable_oil} {cabinet} ?loc ?dim)
)
"""
    )
    sketch_dict["storing food"].append(rule_place_cookable_in_cabinet)
...
# $\{\neg H\} \mapsto \{H\}$  ; pick up a cookable item (width 1)
# $\{H, N_f > 0, is\_opened\} \mapsto \{\neg H, N_f \downarrow\}$  ; place the carried cookable item into some valid cabinet spot (width 1)
...
def create_width_1_sketch(env, map_id, domain_name, window=None):
    ...
    c_cabinet_not_opened = Count(
        type_tag='cabinet',
        wanted_value='>0',
        count_func=partial(count_closed)
    )
    # * effects
...
"""
    ...
    sketch_dict["storing food"].append(rule_open_cabinet)
    # 2. pick up a cookable item (width 1)
    # * precons
    c_cabinet_open_equal_0 = Count(
        type_tag='cabinet',
        wanted_value='=0',
        count_func=partial(count_closed)
    )
    ...
    rule_pick_up_cookable = Rule(
        preconditions=[c_cabinet_open_equal_0, neg_H, n_f_larger_0],
        effects=[H],
        goal_clause_pattern="""
(inhandofrobot agent-01 {chip oatmeal sugar vegetable_oil})
"""
    ...
    sketch_dict["storing food"].append(rule_pick_up_cookable)
    # 3. place the carried cookable item into some valid cabinet spot (width 1)
    # * precons
    H = Holding(
        wanted_value=True,
        type_tag='chip oatmeal sugar vegetable_oil',
    ...
    rule_place_cookable_in_cabinet = Rule(
        preconditions=[H, c_cabinet_open_equal_0, n_f_larger_0],
        effects=[neg_H, n_f_decrease],
        goal_clause_pattern="""
(exists (?loc - location ?dim - dimension)
    (inside {chip oatmeal sugar vegetable_oil} {cabinet} ?loc ?dim)
...
### Sketch with W = 2 ; factor out opening cabinet
# $\{\neg is\_opened\} \mapsto \{is\_opened\}$  ; open the cabinet (width 1)
# $\{N_f > 0, N_c > 0\} \mapsto \{N_f \downarrow, N_c \downarrow\}$  ; move a cookable item into an opened cabinet (width 2)
...
"""
    ...
    sketch_dict["storing food"].append(rule_open_cabinet)
    # 2. move a cookable item into an opened cabinet (width 2)
    # * precons
    c_cabinet_open_equal_0 = Count(
        type_tag='cabinet',
        wanted_value='=0',
    ...
    rule_place_cookable_in_cabinet = Rule(
        preconditions=[c_cabinet_open_equal_0, n_f_larger_0],
...
### Sketch with W = 3
# $\{N_f > 0\} \mapsto \{N_f \downarrow\}$  ; move a cookable item into a cabinet (width 3)
def create_width_3_sketch(env, map_id, domain_name, window=None):
    width = 3
    sketch_dict = {
        "storing food": []
    }
    # 1. move a cookable item into a cabinet (width 3)
    # * precons
    n_f_larger_0 = Count(
        type_tag='chip oatmeal sugar vegetable_oil',
        wanted_value='>0',
...
if __name__ == '__main__':
    main()
    # debug 
    # python /home/xxxname/Project/granularity_instruction_nsai/granularity-instruction-nsai/data/00_envs/mini_behavior/mini_behavior/utils/policy_sketch/storing_food.py --map_id 177901345 --domain_name storing_food --width 1 --rewrite
    # env, map_id, domain_name, window, env_id = init_env_for_policy_sketch('data/00_envs/mini_behavior/mini_behavior/utils/pddl_plans/storing_food/p29769573-storing_food_plans.json', want_render=False)
