...
\begin{figure*}[t]
    ...
    \begin{minipage}[t]{0.56\textwidth}
        ...
        \captionof{figure}{Smaller SR gaps indicate a reliance on vision-dominant, language-ignoring policies. The Pure-C setting exhibits negligible performance variance between (\textit{w/ goal lang}) and (\textit{w/o lang}).}
        \label{fig:sr_diff_by_training_setup}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.40\textwidth}
      \centering
    \includegraphics[width=\linewidth]{figures/q7_loss_comparison.pdf}
    \captionof{figure}{VLM instruction-prediction Cross Entropy (CE) loss across granularities. Larger CE loss indicates lower $P(l \mid v)$.}
...
Why does performance rebound at $w \ge 2$ while ? We show this arises from increasing \emph{instruction predictability} $P(l|v)$, which enables agents to bypass language and exploit visual-dominant policy.

\paragraph{Behavioral Evidence of Modality Collapse.}
Following prior work that uses language ablation to diagnose modality reliance \citep{fei2025liberoplus}, we measure $\Delta\mathrm{SR}_{\text{full - w/o}}$ in \Cref{fig:sr_diff_by_training_setup}. Fine-trained policies collapse without language ($36.3\%$ drop), while coarse-trained ones show a smaller $20.0\%$ gap. Pure-C exhibits a negligible $2.8\%$ difference between static goal descriptions ($15.6\%$) and no language at all ($12.8\%$). This confirms that VLA tends to learn a purely vision-dominant, reactive policies.

\paragraph{Why Language Becomes Ignorable.}
This behavioral shift suggests coarse instructions are simply \emph{easier to predict from vision}. Formally, consider a surjective mapping $f$ from fine‑grained labels $L_{\text{fine}}$ to coarser ones $L_{\text{coarse}}$. The conditional probability of a coarse label aggregates its fine‑grained constituents: $P(c_j | v) = \sum_{l_i \in f^{-1}(c_j)} P(l_i | v)$.
Thus coarse instructions are statistically more predictable from visual context. We confirm this predictability empirically by training a VLM (Qwen3-VL-4B via LoRA) to act as an trained instructor, predicting instructions from visual observations. As \Cref{fig:cross_entropy_loss_comparison} shows, Cross-Entropy loss decreases monotonically with coarseness (Fine: 0.078, Medium: 0.063, Coarse: 0.047), a relative drop of ~40\% from Fine to Coarse. Together, these results form a complementary chain: probability aggregation explains \emph{why} $P(l|v)$ increases, CE loss empirically confirm the increase, and $\Delta$SR quantifies the language ignoring behavior. 
...
\begin{figure*}[t]
    ...
    \begin{minipage}[t]{0.60\textwidth}
        ...
        \captionof{figure}{The x-axis are test instruction width, y-axis are training setup. Transfer is asymmetric: train coarse $\rightarrow$ test fine works, but train fine $\rightarrow$ test coarse does not. Mixed-granularity improves overall.}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Fine ($w=0$):} \emph{Successful Grounding.} Low $P(l|v)$ (high CE loss, \Cref{fig:cross_entropy_loss_comparison}) means language provides unique information. Combined with low-width grounding burden (\Cref{sec:decoupling}), the model reliably maps instructions to actions, yielding high SR (\Cref{fig:q1_learning_curves}).
    ...
    \item \textbf{Coarse ($2 \le w \le 3$):} \emph{Modality Collapse.} High $P(l|v)$ (\Cref{fig:cross_entropy_loss_comparison}) causes the model to exploit visual shortcuts: $\Delta\mathrm{SR}$ approaches 20\% (\Cref{fig:sr_diff_by_training_setup}), which is a signature of modality collapse.

    \item \textbf{Capacity Cap ($w \ge 4$):} \emph{Cognitive Overload.} Tasks requiring $\ge 4$ concurrent features are provably hard for width-based planners (complexity $O(|\Phi|^4)$ \citep{DBLP:conf/aaai/LipovetzkyG17}). We find VLA models hit the same ceiling: performance uniformly collapses at $w \ge 4$ (\Cref{fig:q3_cross_granularity}). This alignment suggests the intrinsic difficulty captured by $w$ translates directly to neural agents, mirroring the theoretical hardness of width-based planning.
\end{itemize}
